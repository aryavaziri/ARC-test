...
const Lookup: React.FC<LookupProps> = ({ field, control, error }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const lookupField = inputFields.find(f => f.id === field.id && f.type === 'lookup');

      if (lookupField && 'lookupModelId' in lookupField) {
        const lookupModelId = lookupField.lookupModelId;
        const result = await getLookupLineItem(lookupModelId);
        setLineItem(result);

        const model = models.find(m => m.id === lookupModelId);
        if (model) {
          setPrimaryFieldId((lookupField as any).primaryFieldId || null);
          const searchColumnIds = (lookupField as any).searchModalColumns?.length ? (lookupField as any).searchModalColumns : [lookupField.primaryFieldId];
